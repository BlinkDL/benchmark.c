...
60.7GB/s for DDR5-6000 dual channel & AMD Zen4 CPU
...
{    
    ...
    flush_cache(array, size);
    
    uint64_t start_ns = get_ns();
    for (size_t i = 0; i < size; i += 64) {
        if (config->use_prefetch) {
            _mm_prefetch((char*)&array[i + config->prefetch_distance], _MM_HINT_T0);
        }        
    ...
    double bandwidth = ((double)size) / ((double)(get_ns() - start_ns)); // here we use 1e9 for 1GB
...
int main()
{
    ...
    {
        printf("[iter %d]", i+1);
        struct benchmark_config config = {64, true, "1 cache line ahead"};
